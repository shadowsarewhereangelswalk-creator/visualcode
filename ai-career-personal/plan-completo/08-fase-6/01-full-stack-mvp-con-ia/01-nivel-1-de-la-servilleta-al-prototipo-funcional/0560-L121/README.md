# L121 — Formularios y Mutaciones de Datos

- **Orden:** 560 de 580
- **Fase:** Fase 6
- **Módulo:** Full Stack MVP con IA
- **Nivel:** Nivel 1: De la Servilleta al Prototipo Funcional
- **Semana sugerida:** 39
- **Lección:** 6
- **Estado:** ⬜ Pendiente
- **Prioridad:** Alta
- **Herramientas:** Next.js, Supabase
- **Duración estimada:** 2 h de clase + 2 h de práctica

## Objetivo de aprendizaje

Manejar creación y edición de registros.

## Conceptos clave

forms, validation, mutation, optimistic UI

## Ejercicio guiado

Crear CRUD simple de proyectos.

## Tarea o entregable

CRUD en app.

## Proyecto asociado

MVP SaaS IA

## Código o práctica disponible

Todavía no se ha agregado una solución. Se incorporará cuando la clase esté en curso o completada y exista un archivo real o una práctica verificada.

## Fuente

Ficha generada desde la fila oficial de `Plan_Clases`. Por privacidad, este repositorio no publica las columnas **Evidencia / URL** ni **Notas**.
